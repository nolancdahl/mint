...
import { TrashIcon, ArrowRightIcon, PenIcon, XIcon, PlusIcon, ClipboardIcon, LinkIcon, TagIcon, TypeIcon, ChevronLeft, ChevronRight, ChevronDown, ShirtIcon } from './Icons'
...
const EditClosetModal = ({ item, onClose, onSave }) => {
  const user = useAuth()
  ...
  return (
    ...
    >
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '16px' }}>
          ...
          <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap', marginBottom: '14px' }}>
            ...
            {sizes.map((s) => (
              <button key={s} onClick={() => toggleSize(s)} style={{
                ...
                border: `1px solid ${selectedSize === s ? COLORS.green : COLORS.greenLine}`,
                background: selectedSize === s ? COLORS.green : COLORS.creamDeep,
                color: selectedSize === s ? COLORS.cream : COLORS.textMuted,
          ...
          <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap', marginBottom: '14px' }}>
            <button onClick={() => setAddCatOpen(true)} style={{
              ...
              cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center',
              flexShrink: 0,
            }}><PlusIcon size={12} strokeWidth={2} /></button>
            {categories.map((c) => (
      ...
      {addTagOpen && createPortal(
        <AddItemPopup label="New category" placeholder="e.g. Chinos, Hats..." onAdd={handleAddTag} onClose={() => setAddTagOpen(false)} />,
        document.body
      )}
      {addColorOpen && createPortal(
        <AddItemPopup label="New color" placeholder="e.g. Coral, Sage..." onAdd={handleAddColor} onClose={() => setAddColorOpen(false)} />,
        document.body
      )}
      {addCatOpen && createPortal(
        <AddItemPopup label="New style" placeholder="e.g. Formal, Streetwear..." onAdd={handleAddCategory} onClose={() => setAddCatOpen(false)} />,
        document.body
      )}
      {addSizeOpen && createPortal(
        <AddItemPopup label="New size" placeholder="e.g. 34x30 slim, 10.5..." onAdd={handleAddSize} onClose={() => setAddSizeOpen(false)} />,
        document.body
      )}
    </div>
  )
...
export const ItemDetailModal = ({ item, onClose, onDelete, onUpdate }) => {
    ...
    >
      ...
      >
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '18px 20px' }}>
          ...
          {((item.categories && item.categories.length > 0) || (item.tags && item.tags.length > 0) || (item.colors && item.colors.length > 0) || item.size) && (() => {
            const pill = {
            ...
            return (
              <div style={{ display: 'flex', marginTop: '16px', borderRadius: '10px', overflow: 'hidden' }}>
                ...
                <div style={{ flex: 1, padding: '10px 6px' }}>
                  ...
                  <div style={{ display: 'flex', gap: '4px', flexWrap: 'wrap', justifyContent: 'center' }}>
                    {item.size && (<span style={{ ...pill, fontWeight: 600 }}>{item.size}</span>)}
                  </div>
                </div>
              </div>
            )
